    ...
    ;(async () => {
      const entries = await Promise.all(
        TOURS.map(async (tour) => {
          const url = await findImage({ query: tour.query, orientation: 'landscape' })
          return [tour.id, url]
        })
      )
            ...
            return (
              <Grid key={tour.id} size={{ xs: 12, sm: 6, md: 4 }}>
                <Box sx={{ borderRadius: 2, overflow: 'hidden', border: '1px solid #eee', bgcolor: '#fff', display: 'flex', flexDirection: 'column', height: '100%' }}>
                  <Box component="img" src={images[tour.id]} alt="" sx={{ width: '100%', height: 200, objectFit: 'cover', display: 'block', bgcolor: '#000' }} />
                  <Box sx={{ p: 2 }}>
                    ...
                    <Typography sx={{ color: 'text.secondary', mt: 0.75 }}>{t(`tours.details.${tour.id}.short`, '')}</Typography>
