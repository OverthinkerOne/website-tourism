...
function BlogCard({ category, title, excerpt, Icon, image, readMoreLabel }) {
  return (
    ...
    >
      {/* Image */}
      <Box sx={{ position: 'relative', height: { xs: '38%', md: '42%' }, minHeight: 120, overflow: 'hidden' }}>
        {image ? (
...
export default function Session7() {
  ...
  return (
    <Box component="section" sx={{ position: 'relative', width: '100%', height: '100vh', overflow: 'hidden', bgcolor: '#F6F3EF' }}>
  ...
  <Box sx={{ position: 'relative', zIndex: 1, width: 'min(1300px, 96vw)', mx: 'auto', py: { xs: 4, md: 6 }, color: '#000', height: '100%', display: 'flex', flexDirection: 'column' }}>
        ...
        <Box sx={{ mt: { xs: 2, md: 3 }, position: 'relative', flexGrow: 1, minHeight: 0, display: 'flex' }}>
          {/* Edge fade masks (softer and narrower to not hide cards) */}
            <Box sx={{ position: 'absolute', inset: 0, pointerEvents: 'none', zIndex: 2, background: 'linear-gradient(90deg, rgba(246,243,239,1) 0%, rgba(246,243,239,0) 4%, rgba(246,243,239,0) 96%, rgba(246,243,239,1) 100%)' }} />
          ...
          <Box ref={viewportRef} onPointerDown={onPointerDown} onPointerMove={onPointerMove} onPointerUp={endDrag} onPointerCancel={endDrag} sx={{ overflowX: 'hidden', overflowY: 'visible', cursor: isDragging ? 'grabbing' : 'grab', touchAction: 'pan-y', height: '100%', width: '100%' }}>
            <Box sx={{ display: 'flex', alignItems: 'stretch', gap: { xs: 2.5, md: 3 }, px: { xs: 1.5, md: 2 }, height: '100%' }}>
              {items.map((post, idx) => {
                ...
                return (
                  <Box key={post.id + '-' + idx} sx={{ height: '100%' }}>
